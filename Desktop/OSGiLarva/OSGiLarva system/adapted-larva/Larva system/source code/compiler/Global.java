...
//				}
	...
	public void createFileForEach()throws ParseException
	{
		...
		//////////Note: now code does not generate aspects
		//////////
		...
		StringBuilder cl = new StringBuilder("package larva;\r\n\r\n"+imports
				+"\r\nimport java.util.LinkedHashMap;" +
						"\r\nimport java.io.PrintWriter;" +
				///////////Date: 5/12/11
						//////////Code modified here for the France version
						//////////Note: now code imports larvaplug.callable
						//////////
				"\r\nimport larvaplug._callable;");
		cl.append("\r\n\r\npublic class _monitor"/*+name*/+id+" implements _callable{");
		
		///////////Date: 3/1/13
		//////////Code do not use name anymore
		//////////Note:
		//////////
		...
		if (this == root)
		{
			cl.append("\r\n\r\npublic static PrintWriter pw; " +
					"\r\npublic static _monitor"+id+" root;");
		}
		...
		cl.append("\r\n\r\npublic static LinkedHashMap<_monitor"/*+name*/+id+",_monitor"/*+name*/+id+"> _cls_monitor"/*+name*/+id
				+ "_instances = new LinkedHashMap<_monitor"/*+name*/+id+",_monitor"/*+name*/+id+">();");
		...
		if (this.id == 0)
		{
			...
			cl.append("\r\npw = new PrintWriter(\""+Compiler.textDir.replace("\\", "\\\\")
					+"output_"+name+".txt\");\r\n");
			
			cl.append("\r\nroot = new _monitor" /*+ this.name*/ + this.id + "();" +
					"\r\n_cls_monitor" /*+ this.name*/ + this.id + "_instances.put(root, root);");
...
//					cl.append("\r\nroot." + v.getVariableName() + ".reset();");
		...
		if (parent != null)
			cl.append("\r\n\r\n_monitor"/*+name */ + parent.id + " parent;");
		else 
			cl.append("\r\n\r\n_monitor"/*+name */ + id + " parent; //to remain null - this class does not have a parent!");
		...
		///////////Date: 5/12/11
		//////////Code commented below for the France version
		//////////Note: 
		///////////Date: 3/1/13
		...
		if (parent != null)
		{
			cl.append("\r\nparent = _monitor"/*+name*/+parent.id+"._get_cls_monitor"/*+name*/+parent.id+"_inst( ");
...
//						cl.append("\r\n" + v.getVariableName() + ".registerCycle(" + l + ");");
		...
		cl.append("\r\n\r\npublic static _monitor"+id+" _get_cls_monitor"+id+"_inst( ");
		for (Variable v: allParentsVars(new ArrayList<Variable>()))
			cl.append(v.type + " " + v.name+ ",");
		cl.delete(cl.length()-1, cl.length());//remove last comma
		cl.append(") { synchronized(_cls_monitor" /*+ this.name*/ + this.id + "_instances){");

        ///////////Date: 3/1/13
		//////////Code setted back
		//////////Note: now code synchronizes on "_cls_monitor" + this.id + "_instances"
        

		if (id != 0)
		{
			cl.append("\r\n_monitor"+id+" _inst = new _monitor"+id+"( ");
			for (Variable v: allParentsVars(new ArrayList<Variable>()))
				cl.append(v.name+ ",");
			cl.delete(cl.length()-1, cl.length());//remove last comma
			cl.append(");");
			cl.append("\r\nif (_cls_monitor"+id+"_instances.containsKey(_inst))");
			cl.append("\r\n{" +
					"\r\n_monitor"+id+" tmp = _cls_monitor"+id+"_instances.get(_inst);");

			for (Invariant inv : invariants.invariants.values())
			{
				cl.append("\r\nif ( tmp." + inv + "_enb && !tmp."+inv+"_temp.equals(" + Tokenizer.showStats(inv.call) +")){"+ 
						"\r\n  _monitor0.pw.println(\" Invariant Check: "+inv+" Failed: " + Tokenizer.showStats(inv.call) + "!!: \" + " +
			...
			cl.append("\r\n return _cls_monitor"+id+"_instances.get(_inst);" +
			"\r\n}");
			cl.append("\r\nelse\r\n{");

			cl.append("\r\n _inst.initialisation();");

			cl.append("\r\n _cls_monitor"+id+"_instances.put(_inst,_inst);" +
					"\r\n return _inst;" +
			"\r\n}");
		}
		else
		...
		cl.append("\r\n\r\npublic boolean equals(Object o) {");
		cl.append("\r\n if ((o instanceof _monitor"+id+")");
		
		for (int i = 0; i < variables.size(); i++)
		{
			if (equateMethods.get(i) == null)
				cl.append("\r\n && ("+variables.get(i).name+" == null || "+variables.get(i).name+".equals(((_monitor"+id+")o)."+variables.get(i).name + "))");
			else
				cl.append("\r\n && ( " + equateMethods.get(i) + "("+variables.get(i).name+", ((_monitor"+id+")o)."+variables.get(i).name + "))");
		}
			
		
		if (parent != null)
			cl.append("\r\n && (parent == null || parent.equals(((_monitor"+id+")o).parent))");
		cl.append(")\r\n{return true;}\r\nelse\r\n{return false;}\r\n}");
		...
		///////////Date: 3/1/13
		//////////Code removed here for the France version
		//////////Note: now code does not refer to events as numbers and name is no more used.
		//////////
		
		//_call one parameter
		//cl.append("\r\n\r\npublic void _call(String _infoSM){" +
		//		"\r\n    synchronized(_cls_monitor" /*+ this.name*/ + this.id + "_instances){");
		//for (Property l:logics.values())
		//	cl.append("\r\n        _performLogic_"+l.name+"(_infoSM);");
		//cl.append("\r\n    }\r\n}");

		//cl.append("\r\n\r\npublic void _call(String _infoSM, Long _callerId, String _infoSI, String _prptyName){" +//DYF/05/12/2013
		cl.append("\r\n\r\npublic void _call(String _infoSISM, Long _callerId, String _infoSI){" +
				"\r\n    synchronized(_cls_monitor" /*+ this.name*/ + this.id + "_instances){");
		for (Property l:logics.values()){
			//if(l.name.equals("client")==false){//DYF/05/12/2013
				//cl.append("\r\n        if(_prptyName.equals(\""+l.name+"\"))");//DYF/05/12/2013
			    cl.append("\r\n        _performLogic_"+l.name+"(_infoSISM);");
			    //cl.append("\r\n        _performLogic_"+l.name+"(_infoSM);");//DYF/05/12/2013
				//}else cl.append("\r\n        _performLogic_"+l.name+"(_infoSM);");//DYF/05/12/2013

		}
		///////////Date: 3/1/13
		//////////Code added here for the France version
		//////////Note: now root call will also call call_all_filtered
		//////////

        if(this==root)
            cl.append("\r\n        _call_all_filtered(_infoSISM, _callerId, _infoSI);");
            //cl.append("\r\n        _call_all_filtered(_infoSM, _callerId, _infoSI, _prptyName);");//DYF/05/12/2013
		...
		///////////Date: 5/12/11
		//////////Code removed here for the France version
		//////////Note: now code does not generate call_all and other methods
		//////////
...
//_call_all_filtered method
		...
		cl.append("\r\n\r\npublic void _call_all_filtered(String _infoSISM, Long _callerId, String _infoSI){");
		//cl.append("\r\n\r\npublic void _call_all_filtered(String _infoSM, Long _callerId, String _infoSI, String _prptyName){");
		//DYF/05/12/2013
		for (Global h: this.foreaches)
		{
//				cl.append("\r\n\r\n    _monitor" /*+ h.name*/ + h.id + "[] a"+h.id+" = new _monitor" /*+ h.name*/ + h.id + "[1];" +
//						"\r\n    synchronized(_monitor" /*+ h.name*/ + h.id + "._cls_monitor" /*+ h.name*/ + h.id + "_instances){" +
//						"\r\n        a"+h.id+" = _monitor" /*+ h.name*/ + h.id + "._cls_monitor" /*+ h.name*/ + h.id + "_instances.keySet().toArray(a"+h.id+");" +
//                      "\r\n    }" +
//						"\r\n    for (_monitor" /*+ h.name*/ + h.id + " _inst : a"+h.id+")" +
//						"\r\n        if (_inst != null");
				cl.append("\r\n\r\n    synchronized(_monitor" /*+ h.name*/ + h.id + "._cls_monitor" /*+ h.name*/ + h.id + "_instances){"+
                          "\r\n        _monitor" /*+ h.name*/ + h.id + " _inst = _monitor" /*+ h.name*/ + h.id + "._get_cls_monitor" /*+ h.name*/ + h.id + "_inst(_callerId, _infoSI);" +
						  "\r\n        if (_inst != null");
...
//					cl.append(" && _inst.parent." + v.getVariableName() + ".equals(" + v.getVariableName()+")");
		//cl.append("){\r\n            _inst._call(_infoSM, _callerId, _infoSI, _prptyName); " +//DYF/05/12/2013
		cl.append("){\r\n            _inst._call(_infoSISM, _callerId, _infoSI); " +
				  "\r\n            _inst._call_all_filtered(_infoSISM, _callerId, _infoSI);"+
				  //"\r\n            _inst._call_all_filtered(_infoSM, _callerId, _infoSI, _prptyName);"+//DYF/05/12/2013
                  "\r\n        }"+
                  "\r\n    }");
		}
		cl.append("\r\n}");
...
///////////Date: 5/12/11
		//////////Code removed here for the France version
		//////////Note: now code does not generate event_occured method
		//////////
		...
		try{
			///////////Date: 5/12/11
			//////////Code removed here for the France version
			//////////Note: now code does not write aspects to files
			//////////
			
		//PrintWriter pw1 = new PrintWriter(Compiler.outputDir+"/aspects/_asp_"+name+id+".aj");
		//pw1.write(as1.toString());
		//pw1.close();
		PrintWriter pw3 = new PrintWriter(Compiler.outputDir+"/larva/_monitor"+id+".java");
...
///////////Date: 6/12/11
		//////////Code removed here for the France version
		//////////Note: now code does not generate callable interface
		//////////
//		createInterfaceCallable();
	...
	public void createClockClass()
	{
		...
		///////////Date: 5/12/11
				//////////Code added here for the France version
				//////////Note: now clock inherits from callable
				//////////
				+"\r\nimport larvaplug._callable;"
				+"\r\n"
				+"\r\npublic class Clock {"
...
//				"\r\n}"+
				...
				"\r\n\r\npublic void send()"+
				"\r\n{"+
				"\r\n	if (on) _monitor0.root._call(\"receiveON\"+name, null, null);"+ // TODO: Check that it's _monitor0.
				"\r\n}"+
...
"\r\npublic class ClockEvent implements Runnable{"+
	...
	public void createInterfaceCallable()
	{
		...
		///////////Date: 5/12/11
		//////////Code removed here for the France version
		//////////Note: now code refers to method name
		//////////
		as1.append("\r\npackage larva;"+
...
//			+"\r\n						synchronized (events){"
			...
			+"\r\n								_monitor0.root._call(c.name+\"AT\"+d, null, null);" // TODO: Check that it's _monitor0
//			+"\r\n						}"
			+"\r\n						}"
			+"\r\n					} "
			+"\r\n				}"
	...
	///////////Date: 5/12/11
			//////////Code removed here for the France version
			//////////Note: now code does not generate aspects folder 
			//////////
	...
	public static String redirectOutput(String stats) {	
		...
		return stats.replace("System .out", "_monitor"+root.id+".pw"); // TODO: Check that it's _monitor0
	}
