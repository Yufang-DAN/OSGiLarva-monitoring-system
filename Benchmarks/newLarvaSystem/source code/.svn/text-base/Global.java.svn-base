...
//				}
	...
	public void createFileForEach()throws ParseException
	{
		...
		cl.append("\r\n\r\npublic class _monitor implements _callable{");
		
///////////Date: 5/12/11
		//////////Code removed here for the France version
		//////////Note: now code always uses the same class name
		//////////
		
		if (this == root)
		{
			cl.append("\r\n\r\npublic static PrintWriter pw; " +
					"\r\npublic static _monitor root;");
		}
		...
		///////////Date: 5/12/11
		//////////Code removed here for the France version
		//////////Note: now code does not generate hashmap
		//////////
		...
		if (this.id == 0)
		{
			...
			///////////Date: 5/12/11
			//////////Code removed here for the France version
			//////////Note: now code does not populate hashmap
			//////////
...
//		if (parent != null)
//		{
//			cl.append("\r\nparent = _mon._get_cls_"+name+parent.id+"_inst( ");
...
//						cl.append("\r\n" + v.getVariableName() + ".registerCycle(" + l + ");");
		...
		cl.append("\r\n\r\npublic static _monitor _get_cls_monitor_inst( ");
		for (Variable v: allParentsVars(new ArrayList<Variable>()))
			cl.append(v.type + " " + v.name+ ",");
		cl.delete(cl.length()-1, cl.length());//remove last comma
		cl.append(") { synchronized(root){");
		///////////Date: 5/12/11
		//////////Code removed here for the France version
		//////////Note: now code synchronises on "root"
		//////////

		if (id != 0)
		{
			cl.append("\r\n_monitor _inst = new _monitor( ");
			for (Variable v: allParentsVars(new ArrayList<Variable>()))
				cl.append(v.name+ ",");
			cl.delete(cl.length()-1, cl.length());//remove last comma
			cl.append(");");
			cl.append("\r\nif (_monitor_instances.containsKey(_inst))");
			cl.append("\r\n{" +
					"\r\n_monitor tmp = _monitor_instances.get(_inst);");

			for (Invariant inv : invariants.invariants.values())
			{
				cl.append("\r\nif ( tmp." + inv + "_enb && !tmp."+inv+"_temp.equals(" + Tokenizer.showStats(inv.call) +")){"+ 
						"\r\n  _monitor.pw.println(\" Invariant Check: "+inv+" Failed: " + Tokenizer.showStats(inv.call) + "!!: \" + " +
			...
			cl.append("\r\n return _monitor_instances.get(_inst);" +
			"\r\n}");
			cl.append("\r\nelse\r\n{");

			cl.append("\r\n _inst.initialisation();");

			cl.append("\r\n _monitor_instances.put(_inst,_inst);" +
					"\r\n return _inst;" +
			"\r\n}");
		}
		else
		...
		cl.append("\r\n\r\npublic boolean equals(Object o) {");
		cl.append("\r\n if ((o instanceof _monitor)");
		
		for (int i = 0; i < variables.size(); i++)
		{
			if (equateMethods.get(i) == null)
				cl.append("\r\n && ("+variables.get(i).name+" == null || "+variables.get(i).name+".equals(((_monitor)o)."+variables.get(i).name + "))");
			else
				cl.append("\r\n && ( " + equateMethods.get(i) + "("+variables.get(i).name+", ((_monitor)o)."+variables.get(i).name + "))");
		}
			
		
		if (parent != null)
			cl.append("\r\n && (parent == null || parent.equals(((_monitor)o).parent))");
		cl.append(")\r\n{return true;}\r\nelse\r\n{return false;}\r\n}");
		...
		///////////Date: 5/12/11
		//////////Code removed here for the France version
		//////////Note: now code does not refer to events as numbers
		//////////
		cl.append("\r\n\r\npublic void _call(String _info){" +
				"\r\nsynchronized(root){");
		for (Property l:logics.values())
			cl.append("\r\n_performLogic_"+l.name+"(_info);");
		cl.append("\r\n}\r\n}");


		///////////Date: 5/12/11
		//////////Code removed here for the France version
		//////////Note: now code does not generate call_all_filtered and other methods
		//////////
...
///////////Date: 5/12/11
		...
		//pw1.close();
		PrintWriter pw3 = new PrintWriter(Compiler.outputDir+"/larva/_monitor.java");
...
//				"\r\n}"+
				...
				"\r\n\r\npublic void send()"+
				"\r\n{"+
				"\r\n	if (on) _monitor.root._call(\"receiveON\"+name);"+
				"\r\n}"+
...
//			+"\r\n						synchronized (events){"
			...
			+"\r\n								_monitor.root._call(c.name+\"AT\"+d);"
//			+"\r\n						}"
			+"\r\n						}"
			+"\r\n					} "
			+"\r\n				}"
	...
	public static String redirectOutput(String stats) {	
		...
		return stats.replace("System .out", "_monitor.pw");
	}
